...
const generalAlternatives: Alternative[] = [
  { id: "ojol", name: "Ojek Online", cost: 20000, time: 25, distance: 8, icon: Bike },
  { id: "taksi", name: "Taksi Online", cost: 55000, time: 35, distance: 8, icon: CarFront },
  { id: "krl", name: "KRL Commuter Line", cost: 4000, time: 20, distance: 10, icon: Train },
  { id: "bus", name: "Bus / TransJakarta", cost: 3500, time: 45, distance: 9, icon: Bus },
  { id: "combo1", name: "Ojek Online + KRL", cost: 15000, time: 22, distance: 9, icon: Layers, isCombo: true },
];
...
export default function Dashboard() {
  ...
  // ----------------------------------------------------
  const handleWeightChange = (changedKey: "cost" | "time" | "distance", newValue: number) => {
    const clampedValue = Math.min(100, Math.max(0, newValue));
    const otherKeys = (["cost", "time", "distance"] as const).filter((k) => k !== changedKey);
    ...
    if (currentOtherSum > 0) {
      newOther1 = Math.round((weights[otherKeys[0]] / currentOtherSum) * remaining);
      ...
      <div className="min-h-screen bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-white transition-colors p-4 md:p-6">
        ...
        <div className="bg-gradient-to-r from-cyan-500 to-blue-600 dark:from-cyan-700 dark:to-blue-900 rounded-3xl p-8 text-white mb-6 relative overflow-hidden">
          <div className="max-w-2xl">
            ...
            <p className="text-cyan-100">
              Platform pintar untuk membantu Anda memilih moda transportasi terbaik (tunggal maupun kombinasi multimoda) berdasarkan prioritas personal Anda.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm transition-colors border border-slate-200/50 dark:border-slate-700/50">
            <div className="flex items-center gap-2 mb-3 text-cyan-500">
              <Info size={22} />
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Tentang Website Ini</h2>
            </div>
            <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
              Website ini dirancang menggunakan algoritma Simple Additive Weighting (SAW) untuk memberikan rekomendasi objektif pilihan transportasi. Sistem secara otomatis memperhitungkan kombinasi biaya, efisiensi waktu, dan jarak perjalanan sesuai bobot preferensi yang Anda tentukan sendiri.
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm transition-colors border border-slate-200/50 dark:border-slate-700/50">
            <div className="flex items-center gap-2 mb-3 text-cyan-500">
              <HelpCircle size={22} />
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Cara Penggunaan</h2>
            </div>
            <ul className="text-sm text-slate-600 dark:text-slate-300 space-y-2">
              <li className="flex items-start gap-2">
                <span className="font-bold text-cyan-500">3.</span>
                <span>Klik tombol "Input Alternatif Anda" untuk menambahkan rute atau pilihan moda transportasi kustom Anda sendiri.</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-cyan-500">2.</span>
                <span>Sistem akan langsung menghitung ulang Skor Akhir & Peringkat secara real-time.</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-cyan-500">2.</span>
                <span>Pilih tombol menentukan mana yang lebih penting: Biaya, Waktu, atau Jarak.</span>
              </li>
              <li className="flex items-start gap-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-5 mb-6">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm transition-colors">
            <p className="text-slate-500 dark:text-slate-400 text-sm">Alternatif Opsi</p>
            <h2 className="text-3xl font-bold mt-2">7</h2>
            <CarFront className="text-cyan-500 mt-3" />
          </div>

          <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm transition-colors">
            <p className="text-slate-500 dark:text-slate-400 text-sm">Status Geolocation</p>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm transition-colors">
            <p className="text-slate-500 dark:text-slate-400 text-sm">Rekomendasi Terbaik</p>
            <h2 className="text-2xl font-bold mt-2 text-emerald-500 truncate">
              {bestOption?.name || "-"}
            </h2>
            {bestOption && <bestOption.icon className="text-emerald-500 mt-3" />}
          </div>

          <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm transition-colors">
            <p className="text-slate-500 dark:text-slate-400 text-sm">Skor Kelayakan</p>
            <h2 className="text-3xl font-bold mt-2">{bestOption?.scorePercentage}%</h2>
            <TrendingUp className="text-orange-500 mt-3" />
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm transition-colors">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-xl font-bold">Peta Interaktif Transportasi</h2>
                  <p className="text-slate-500 dark:text-slate-400 text-sm">{locationStatus}</p>
                </div>
  ...
  <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed mb-4">
    Sistem keputusan ini menggunakan kalkulasi objektif berbasis matematika:
  </p>
  ...
  <ul className="space-y-3 text-xs text-slate-600 dark:text-slate-400">
    <li className="flex items-start gap-2.5">
      ...
      <span>
        <strong className="text-slate-800 dark:text-slate-200">Normalisasi Cost:</strong> Semua parameter (Biaya, Waktu, Jarak) bersifat <em>cost</em> (makin kecil makin baik).
      </span>
    </li>
    <li className="flex items-start gap-2.5">
      <CheckCircle2 size={16} className="text-emerald-500 shrink-0 mt-0.5" />
      <span>
        <strong className="text-slate-800 dark:text-slate-200">Pembobotan Dinamis:</strong> Nilai ternormalisasi dikalikan dengan bobot persentase slider Anda secara langsung.
      </span>
    </li>
    <li className="flex items-start gap-2.5">
      <CheckCircle2 size={16} className="text-emerald-500 shrink-0 mt-0.5" />
      <span>
        <strong className="text-slate-800 dark:text-slate-200">Skor Akhir:</strong> Penjumlahan seluruh kriteria menghasilkan peringkat kelayakan dari 0% hingga 100%.
      </span>
    </li>
  </ul>
</div>
  ...
  <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed mb-4">
    Berdasarkan pengaturan bobot Anda saat ini, fokus utama sistem difokuskan pada:
  </p>
  ...
  <div className="space-y-3 text-xs">
    <div className="p-3 rounded-2xl bg-cyan-50 dark:bg-cyan-950/40 border border-cyan-200 dark:border-cyan-800/50">
      ...
      <p className="text-slate-600 dark:text-slate-400">
        Makin murah opsi transportasi, makin tinggi kontribusi nilai kelayakannya.
      </p>
    </div>
    ...
    <div className="p-3 rounded-2xl bg-violet-50 dark:bg-violet-950/40 border border-violet-200 dark:border-violet-800/50">
      ...
      <p className="text-slate-600 dark:text-slate-400">
        Makin cepat estimasi durasi sampai, makin besar skor efisiensinya.
      </p>
    </div>
    ...
    <div className="p-3 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/50">
      ...
      <p className="text-slate-600 dark:text-slate-400">
        Memilih rute terpendek untuk meminimalkan kelelahan di perjalanan.
      </p>
    </div>
  </div>
</div>
